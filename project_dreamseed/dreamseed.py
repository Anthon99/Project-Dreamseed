...
SPELL_CAST_X = 200
SPELL_CAST_Y = 400
LIGHTNING_CAST_X = random.randrange(1, 1000)
LIGHTNING_CAST_Y = 1000
...
class MyGame(arcade.Window):
        ...
        def update(self, delta_time):
                ...
                for lightning in self.spell_list:
                        if lightning.bottom > self.width or lightning.top < 0 or lightning.right < 0 or lightning.left > self.width:
                                lightning.kill()
        ...
        def on_mouse_press(self, x, y, button, key_modifiers):
                ...
                if (self.selected_spell == 1) and (self.magic_resource_percentage > (self.magic_resource_spend_modifer * self.firefury_cost)):
                        ...
                        lightning = arcade.Sprite("images/spells/spell_lightning.png", CHARACTER_SCALING)
                        lightning.center_x = LIGHTNING_CAST_X
                        lightning.center_y = LIGHTNING_CAST_Y
                        self.magic_resource_percentage = self.magic_resource_percentage - self.lightning_cost

                        destination_x = x
                        destination_y = y

                        difference_in_x = destination_x - LIGHTNING_CAST_X
                        difference_in_y = destination_y - LIGHTNING_CAST_Y
                        angle = math.atan2(difference_in_y, difference_in_x)

                        #unlike other spells, lightning comes from the top of the screen
                        lightning.angle = 360

                        lightning.change_x = math.cos(angle) * LIGHTNINGSPEED
                        lightning.change_y = math.sin(angle) * LIGHTNINGSPEED

                        self.spell_list.append(lightning)
