...
class MyGame(arcade.Window):
		...
		def __init__(self):
				...
				self.enem_enemyranged_list = None
				self.base_list = None
				self.gui_list = None
		...
		def setup(self):
				...
				self.enem_enemyranged_list = arcade.SpriteList()
		...
		def on_draw(self):
				...
				self.enem_enemyranged_list.draw()
		...
		def update(self, delta_time):
				...
				self.enem_enemyranged_list.update()
				...
				for enemyranged in self.enem_enenmyrange_list:
						if enemyrange.bottom > self.width or enemyranged.top < 0 or enemyranged.right < 0 :
															
								print (enemyranged.health)
								enemyranged.kill()
								print ("Enemyranged deleted")
						if enemyranged.health < 0:
							enemyranged.kill()
							print ("Enemyranged DESTROYED")
							
						if enemyranged.center_x < 700:
							enemyranged.change_x = 0
						...
						enemyranged_fire_hit_list = arcade.check_for_collision_with_list(enemyranged, self.spell_firefury_list)
						enemyranged_water_hit_list = arcade.check_for_collision_with_list(enemyranged, self.spell_waterblast_list)
						enemyranged_lightning_hit_list = arcade.check_for_collision_with_list(enemyranged, self.spell_lightning_list)
						
						if len(enemyranged_fire_hit_list) > 0:
							enemyranged.health -= self.firefury_damage
												
						for firefury in enemyranged_fire_hit_list:
							enemyranged.remove_from_sprite_lists()
							
						if len(enemyranged_water_hit_list) > 0:
							enemyranged.health -= self.waterblast_damage
												
						for waterblast in enemyranged_water_hit_list:
							enemyranged.remove_from_sprite_lists()
							
						if len(enemyranged_lightning_hit_list) > 0:
							enemyranged.health -= self.lightning_damage
												
						for lightning in enemyranged_lightning_hit_list:
							lightning.remove_from_sprite_lists()
				...
				if self.enem_pool_enemyranged > 0 and self.enem_gap_enemyranged <0:
					enemyranged = arcade.sprite("images/enemies/enemyranged.png")
					enemyranged.center_x = 1800
					enemyranged.center_y = random.triangular(50,300,125)
					enemyranged.change_x = 900
					enemyranged.health = random.randint(10, 50)
					self.enem_pool_enemyranged -=1
					self.enem_gap_enemyranged = random.radint(180,600)
